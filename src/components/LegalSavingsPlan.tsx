...
═══════════════════════════════════════════════════════════════════
Generated by VisaGPT - Legal Savings Plan Advisor
═══════════════════════════════════════════════════════════════════
    `.trim()
    ...
    <div className="min-h-screen px-4 pt-20 pb-28 relative z-10">
      <div className="max-w-lg mx-auto">
          <motion.div
            ...
            className="mb-6"
          >
            <h2 className="text-2xl font-bold mb-2 gradient-text">{t('legalSavingsPlan')}</h2>
            <p className="text-white/60 text-sm">{t('legalSavingsPlanDesc')}</p>
          </motion.div>
            ...
            <>
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-bold text-sm">{t('whyImportant')}</h3>
              ...
              >
                <div>
                  <label className="text-sm text-white/70 mb-2 block">{t('currentBankBalance')} (DZD)</label>
                ...
                <div>
                  <label className="text-sm text-white/70 mb-2 block">{t('monthlyIncome')} (DZD)</label>
                  <div className="relative">
                    <DollarSign className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                    <input
                      type="number"
                ...
                <div>
                  <label className="text-sm text-white/70 mb-2 block">{t('monthlyExpenses')} (DZD)</label>
                  <div className="relative">
                    <DollarSign className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                    <input
                      type="number"
                ...
                <div>
                  <label className="text-sm text-white/70 mb-2 block">{t('requiredAmountVisa')} (DZD)</label>
                  <div className="relative">
                  ...
                  <p className="text-xs text-white/50 mt-1">{t('schengenNote')}</p>
                </div>

                <div>
                  <label className="text-sm text-white/70 mb-2 block">{t('plannedTripDate')}</label>
                  <div className="relative">
          ...
          {step === 2 && (
            ...
            >
              <div className="text-center mb-6">
                ...
                <h3 className="font-bold text-lg">{t('calculatingPlan')}</h3>
              </div>
            </motion.div>
          )}
          ...
          >
            <div className={cn(
              ...
              <h3 className="font-bold text-lg mb-1">
                {plan.dangerLevel === 'safe' && t('safeLevel')}
                {plan.dangerLevel === 'moderate' && t('moderateLevel')}
                {plan.dangerLevel === 'risky' && t('riskyLevel')}
              </h3>
              
              <p className="text-sm text-white/60">
                {t('savingsRatio')}: {((plan.monthlySavings / plan.monthlyIncome) * 100).toFixed(0)}% {t('ofIncome')}
              </p>
            </div>
            ...
            <div className="glass-card p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-white/5 rounded-lg">
                  <p className="text-xs text-white/50 mb-1">{t('currentBalance')}</p>
                  <p className="font-bold text-lg">{plan.currentBalance.toLocaleString()}</p>
                </div>
                <div className="text-center p-3 bg-white/5 rounded-lg">
                  <p className="text-xs text-white/50 mb-1">{t('target')}</p>
                  <p className="font-bold text-lg">{plan.targetAmount.toLocaleString()}</p>
                </div>
                <div className="text-center p-3 bg-white/5 rounded-lg">
                  <p className="text-xs text-white/50 mb-1">{t('monthlySavings')}</p>
                  <p className="font-bold text-lg text-green-400">+{plan.monthlySavings.toLocaleString()}</p>
                </div>
                <div className="text-center p-3 bg-white/5 rounded-lg">
                  <p className="text-xs text-white/50 mb-1">{t('expectedAfter')} {plan.months} {t('months')}</p>
                  <p className={cn(
                    'font-bold text-lg',
              ...
              {plan.projectedBalance >= plan.targetAmount ? (
                <div className="mt-4 p-3 bg-green-500/10 rounded-lg text-center">
                  ...
                  <span className="text-green-400 font-medium">{t('willReachGoal')}</span>
                </div>
              ) : (
                <div className="mt-4 p-3 bg-yellow-500/10 rounded-lg text-center">
                  ...
                  <span className="text-yellow-400 font-medium">
                    {t('needExtra')} {(plan.targetAmount - plan.projectedBalance).toLocaleString()}
                  </span>
                </div>
              )}
            </div>
                ...
                {plan.timeline.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-white/50">{language === 'ar' ? item.actionAr : item.action}</p>
                    </div>
            ...
            <div className="glass-card p-4">
              ...
              <div className="space-y-2">
                {(language === 'ar' ? plan.tipsAr : plan.tips).map((tip, i) => (
            ...
            <div className="glass-card p-4 border-red-500/30">
              ...
              <div className="space-y-2 text-sm text-white/70">
                <div className="flex items-start gap-2">
                  ...
                  <span>{t('redFlag1')}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-red-400">✗</span>
                  <span>{t('redFlag2')}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-red-400">✗</span>
                  <span>{t('redFlag3')}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-red-400">✗</span>
                  <span>{t('redFlag4')}</span>
                </div>
              </div>
            </div>
